...
IQAIR_TABLE = os.environ.get("IQAIR_TABLE") if os.environ.get("IQAIR_TABLE") else 'iqair'
...
def saveFakerDf(dfrecords):
    ...
    cqlsentence = "INSERT INTO " + fakertable + " (id, street, streetname, buildingnumber, city, zipcode, country, countrycode, latitude, longtitude) \
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    batch = BatchStatement(consistency_level=ConsistencyLevel.QUORUM)
    insert = session.prepare(cqlsentence)
    batches = []
    for idx, val in dfrecords.iterrows():
        batch.add(insert, (val['id'], val['street'],
                           val['streetname'], val['buildingnumber'], val['city'], val['zipcode'], val['country'], val['countrycode'], val['latitude'], val['longitude']))
        counter += 1
        if counter >= 100:
            print('inserting ' + str(counter) + ' records')
            totalcount += counter
...
def saveIQAirDf(dfrecords):
    ...
    cqlsentence = "INSERT INTO " + iqairtable + " (city, state, country, latitude, longitude, current_ts, current_pollution_aqius, current_pollution_mainus, current_pollution_aqicn, current_pollution_maincn, current_weather_tp, current_weather_pr, current_weather_hu, current_weather_ws, current_weather_wd, current_weather_ic) \
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    batch = BatchStatement(consistency_level=ConsistencyLevel.QUORUM)
    insert = session.prepare(cqlsentence)
    batches = []
    for idx, val in dfrecords.iterrows():
        batch.add(insert, (val['city'], val['state'],
                           val['country'], val['latitude'], 
                           val['longitude'], val['current_ts'], 
                           val['current_pollution_aqius'], val['current_pollution_mainus'], 
                           val['current_pollution_aqicn'], val['current_pollution_maincn'], 
                           val['current_weather_tp'], val['current_weather_pr'], 
                           val['current_weather_hu'], val['current_weather_ws'], 
                           val['current_weather_wd'], val['current_weather_ic']))
        counter += 1
        if counter >= 100:
            print('inserting ' + str(counter) + ' records')
            totalcount += counter
...
def loadDF(targetfile, target):
    if target == 'weather':
        ...
        colsnames = ["id", "street", "streetname", "buildingnumber", "city", "zipcode", "country", "countrycode", "latitude", "longtitude"]
        dfData = pd.read_csv(targetfile, header=None,
                             parse_dates=True, names=colsnames)
        saveFakerDf(dfData)
    elif target == 'iqair':
        colsnames = ['city', 'state', 'country', 'latitude', 'longitude', 'current_ts', 'current_pollution_aqius', 'current_pollution_mainus', 'current_pollution_aqicn', 'current_pollution_maincn', 'current_weather_tp', 'current_weather_pr', 'current_weather_hu', 'current_weather_ws', 'current_weather_wd', 'current_weather_ic']
        dfData = pd.read_csv(targetfile, header=None,
                             parse_dates=True, names=colsnames)
        saveIQAirDf(dfData)
...
def getDF(source_table):
    ...
    if source_table not in (WEATHER_TABLE, TWITTER_TABLE, FAKER_TABLE, IQAIR_TABLE):
        return None
